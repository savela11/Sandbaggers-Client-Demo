<template>
  <div class='navMenu'>
    <div class='navMenu__container'>
      <div class='navMenu__landmark navMenu__landmark--header'>
        <div class='navMenu__section navMenu__section--header' :style='{ backgroundColor: Header.bgColor }'>
          <div class='navMenu__logo'>
            <img src='@/assets/SBLogo.png' alt='Sandbagger Logo' />
          </div>
          <div class='navMenu__user-profile'>
            <div class='navMenu__user-profile__messages'>
              <IconBtn class='navMenu__user-profile__message' class-name='circle white'>
                <template v-slot:svg>
            ...
            <div class='navMenu__user-profile__image-container' @click.prevent.stop='closeNavMenu'>
              <router-link class='navMenu__user-profile__link' to='/UserProfile'>
                <img :src='currentUserImage' alt='Profile Image' /></router-link>
            </div>
          </div>

        </div>
      </div>
      <div class='navMenu__landmark navMenu__landmark--main'>
        <div class='navMenu__section navMenu__section--admin-buttons' v-if="AdminLinks.length > 0 || AdditionalLinks.length > 0">
          <button @click="toggleLinksView('Main')" :class="{ active: currentLinkView === 'Main' }">Main
          </button>
          <button v-if="AdminLinks.length > 0" @click="toggleLinksView('Admin')" :class="{ active: currentLinkView === 'Admin' }">Admin
          </button>
          <button v-if='AdditionalLinks.length > 0'  @click="toggleLinksView('Additional')" :class="{ active: currentLinkView === 'Additional' }">Additional
          </button>
        </div>
        ...
        <nav>
          ...
          <transition name='slide-fade'>
            <ul v-show="currentLinkView === 'Additional'">
              <li v-for='link in AdditionalLinks' :key='link.name' @click.prevent.stop='closeNavMenu'>
                <router-link :to='link.link'>
                  <div class='svgContainer' v-html='link.icon'></div>
                  <span class='text text--xs'>{{ link.name }}</span>
                </router-link>
      ...
      <div class='navMenu__landmark navMenu__landmark--footer'>
        <button id='logoutBTN' @click='logout' class='navMenu__btn navMenu__btn--logout'>Logout</button>
        <button class='navMenu__btn navMenu__btn--close-menu' @click.prevent.stop='closeNavMenu'>Close</button>
      </div>
    </div>
  </div>
</template>
...
@Component({
  name: 'NavMenu', components: {
    AuthHeader: (): Promise<typeof import('*.vue')> => import('@/layouts/components/AuthHeader.vue'),
...
export default class NavMenu extends Vue {
  ...
  get AdminLinks(): Array<UserLink> {
    const links = this.$store.getters['navigationStore/AdminLinks']

    return links.filter((l: UserLink) => {
  ...
  get AdditionalLinks(): Array<UserLink> {
    const links = this.$store.getters['navigationStore/AdditionalLinks']
    return links.filter((l: UserLink) => {
      if (this.currentUser && this.currentUser.roles.length > 0) {
        if (this.currentUser.roles.includes('Admin')) {
          return l
        } else {
          if (this.currentUser.roles.includes(l.requiredRole)) {
            return l
          }
        }
...
@use "~@/assets/styles/layouts/components/_navMenuAlt.scss";
</style>
