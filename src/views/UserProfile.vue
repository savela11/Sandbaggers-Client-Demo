<template>
  <div class="userProfile" v-if="currentUser && currentUser.profile">
    ...
    <section class="section section__middle">
      <div class="userProfile__handicap column">
        <p>{{ currentUser.profile.handicap }}</p>
        <h3>Handicap</h3>
      </div>
    </section>
    ...
    <section class="section section__bottom" v-if="!loading">
      ...
      <!--      Handicap-->
      <form v-show="currentView.title === 'Handicap'" class="view handicap">
        <div class="form__field">
          <label for="handicap">Handicap</label>
          <input type="number" id="handicap" max="50" min="0" step="0.01"
            v-model.number.trim="currentUser.profile.handicap" />
        </div>
      </form>
      <!--      Update Button-->
...
export default class UserProfile extends Vue {
  ...
  formatPhone(): void {
    const x = this.currentUser.phoneNumber.replace(/\D/g, '').match(/(\d{0,3})(\d{0,3})(\d{0,4})/) as RegExpMatchArray
    this.currentUser.phoneNumber = !x[2] ? x[1] : '(' + x[1] + ') ' + x[2] + (x[3] ? '-' + x[3] : '')
  }
...
.section {
  ...
  &__middle {
    ...
    h3 {
      font-weight: normal;
      @include font-size($--columnTitleFS);
      letter-spacing: 1px;
    }
    ...
    p {
      ...
      font-weight: bold;
      @include font-size($--columnParaFS);
    }
