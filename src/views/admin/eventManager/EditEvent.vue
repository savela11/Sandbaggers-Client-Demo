...
@Component({
  name: 'EditEvent',
  components: {
    EditTeam: (): Promise<typeof import('*.vue')> => import('@/views/admin/eventManager/_EditTeam.vue'),
...
export default class EditEvent extends Vue {
  ...
  async UpdateTeams(teams: Array<TeamVm>): Promise<void> {
    this.loading = true
    try {
      const res = await TeamManagerService.UpdateTeams(teams)
      if (res.status === 200) {
        UIHelper.SnackBar({
          title: 'Success',
  ...
  async UpdateEvent(): Promise<void> {
    this.loading = true
    try {
      const res = await EventManagerService.UpdateEvent(this.Event)
      if (res.status === 200) {
        UIHelper.SnackBar({
          title: 'Success',
  ...
  async getEvent(): Promise<void> {
    try {
      const res = await EventManagerService.Event(parseInt(this.$route.params.id))
      if (res.status === 200) {
        this.Event = res.data
      }
    } catch (e) {
  ...
  async registration(user: RegisteredUserVm, status?: string): Promise<void> {
    ...
    try {
      ...
      if (status === 'register') {
        res = await EventManagerService.RegisterUserForEvent(registerUserForEventDto)
        userIndex = this.Event.unRegisteredUsers.findIndex((u) => u.id === registerUserForEventDto.userId)
      } else {
        res = await EventManagerService.UnRegisterUserFromEvent(registerUserForEventDto)
