<template>
  <div class="view">
    ...
    <div class="powerRankings" v-if="!loading">
      ...
      <!--      <div >-->
      <!--        <router-link :to="`/powerRankings/createPowerRanking/${selectedYear.eventId}`" class="createPowerRankingBtn">Create</router-link>-->
      <!--      </div>-->
      <div class="currentViewBtns">
        <button v-for="option in currentViewOptions" :key="option" :class="{active: currentView === option}" @click="currentView = option"> {{ option }}</button>
      </div>
      <div class="views" v-if="selectedYear">
        <div v-show="currentView === 'Rankings'" class="view rankings">
          <div class="titleBar">
            <div>
              <p>Rank</p>
            </div>
        ...
        <div v-show="currentView === 'Registered Users'" class="view registeredUsers">
          <div class="userList" v-if="selectedYear && selectedYear.registeredUsers.length > 0">
            <div class="user" :class="{ active: user.id === showUsersWriteUpID }" v-for="user in selectedYear.registeredUsers" :key="user.id">
              <div class="top">
                <div class="sandbagger">
                  <div class="name">
                    <h3>{{ user.fullName }}</h3>
                  ...
                  <div>
                    <router-link class="createPowerRankingBtn" :to="{ name: 'CreatePowerRanking', params: { eventId: selectedYear.eventId, userId: user.id }}">Add Power Ranking</router-link>
                  </div>
                </div>
              </div>
            </div>
...
export default class PowerRankings extends Vue {
  ...
  currentViewOptions = ["Rankings", "Registered Users"];
  ...
  async getPowerRankings(): Promise<void> {
    try {
      ...
      if (res.status === 200) {
        ...
        if (this.selectedYear.rankings.length < 1) {
          this.currentView = "Registered Users";
        }
      }
    } catch (e) {
      console.log(e);
