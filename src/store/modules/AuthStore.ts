...
const mutations = {
  ...
  UpdateUserProfileImage(state: IAuthState, profileImage: string):void {
    if(state.currentUser) {
      state.currentUser.image = profileImage;
    }
  }
}
...
const actions = {
  ...
  async UpdateUserSettings(context: ActionContext<IAuthState, IRootState>, userSettings: UserSettingsVm): Promise<void> {
    if (context.state.currentUser) {
      ...
      await context.dispatch('authStore/Logout')

    }
  },

  async UpdateUserProfileImage(context: ActionContext<IAuthState, IRootState>, profileImage: string): Promise<void> {
    if (context.state.currentUser) {
      context.commit('UpdateUserProfileImage', profileImage)
    } else {
      await context.dispatch('authStore/Logout')
    }
